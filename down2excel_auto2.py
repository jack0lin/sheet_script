...
def ck():
	#根据ck表格cookie创建请求头
	wb_ck=openpyxl.load_workbook('ck.xlsx')
	ws_ck=wb_ck['Sheet']
			...
			for contract in list_contract:
				...
				try:
					...
					if shopname in wb.get_sheet_names():
						...
						ws.append(['日期','推广通费用（元）','团购订单数（个）','总订单数（个）','广告点击数量（元/次）','感兴趣数量（次）','图片点击数量（次）','评论点击数量（次）','团购点击数量（次）','浏览数量（次）','曝光数量（次）','实际获客成本（元）','点击均价','午间台数','下午茶台数','晚市台数','总实收金额','套餐实收金额','代金券实收金额','美团收单金额','新增评价数（点评）','新增差评数（点评）','新增精选评价数（点评）','星级评分（点评）','新增评价数量（美团）','新增差评数（美团）','星级评分（美团）','线上活动','推广通调整动作','人气榜（美团）','点评品类热门榜','点评全美食热门榜（全城不分品种总排位）'])
						try:
